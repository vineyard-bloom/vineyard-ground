...
const scheming_1 = require("./scheming");
function sync_collections(schema, collections, keys, sequelize_models, client) {
    for (let name in keys) {
        ...
        collections[name] = new collection_1.Collection(schema.tables, trellis, table, client);
    }
}
...
class Modeler {
    constructor(schema, client) {
        this.collections = {};
        this.schema = schema instanceof scheming_1.SchemaClass
            ? schema
            : new scheming_1.SchemaClass(schema);
